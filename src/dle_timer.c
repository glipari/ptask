...
static void dle_manager_handler(int signo, siginfo_t *info, void * context)
{
    ...
    printf("Handler Task Manager Warning : task %d received signal %d", info->si_value.sival_int, signo);
    printf("\n\tNow %ld, Deadline %ld\n", ptask_gettime(MILLI), tspec_to_rel(&task->dl, MILLI));
...
int dle_init()
{
    ...
    sigaddset(&manager_sigaction.sa_mask, dle_timers[task_index].dle_timer_signo);
    assert(sigaction(dle_timers[task_index].dle_timer_signo, &manager_sigaction, NULL) == 0);
    ...
    return timer_create(CLOCK_MONOTONIC, &sev, &dle_timers[task_index].dle_timer_timerid);
}
...
{
    int task_index = ptask_get_index();
    if(!dle_timer_initialized(task_index)) return -1;
...
int dle_timer_start()
{
    ...
    printf("Arm %ld, task %d | Deadline %ld\n", ptask_gettime(MILLI), task_index, tspec_to_rel(&task->dl, MILLI));
    return timer_settime(dle_timers[task_index].dle_timer_timerid, TIMER_ABSTIME, &its, NULL);
}
...
int dle_timer_stop()
{
    ...
    printf("Disarm %ld, task %d | Deadline %ld\n", ptask_gettime(MILLI), task_index, tspec_to_rel(&ptask_get_task(task_index)->dl, MILLI));
    return timer_settime(dle_timers[task_index].dle_timer_timerid, TIMER_ABSTIME, &its, NULL);
}
