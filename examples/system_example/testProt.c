...
char buf[100];
	...
	textout_ex(screen, font, "Normal Exe", XMIN_GRID+40, 	30, 	COLOR_NORM_EXE, BGC);
	textout_ex(screen, font, "resource A", XMIN_GRID+40+100, 30, 	COLOR_SEZ_A, BGC);
	textout_ex(screen, font, "resource B", XMIN_GRID+40+200, 30, 	COLOR_SEZ_B, BGC);
	...
	for (i = 1 ; i < num_tasks; i++) {
		...
		do {
			at = k * period[i];
			x = OFFSET + at/scale;
			if (x < XMAX_GRID) line(screen, x, lev, x, lev-20, TLCOL);
...
void	task() {
    ...
    while(x < XWIN){
		...
		if (resource[i]==SEZ_A) pthread_mutex_unlock(&mx_sezA);

		if (resource[i]==SEZ_B)	pthread_mutex_unlock(&mx_sezB);
