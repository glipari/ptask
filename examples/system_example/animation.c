...
void periodicBall_testPart(){
		...
		textprintf_ex(screen, font, XMIN+10, BASE1+30+(15*i), FGC, BGC, " T%d run on core - %d -",i+1,sched_getcpu());
		...
		while (1) {
		   ...
		   if (x > XMAX) {
			...
			draw_ball(ox, y, BGC);
			draw_ball(x, y, col);
			textprintf_ex(screen, font, XMIN+10+144 , BASE1+30+(15*i), FGC, BGC, "%d", sched_getcpu());
			pthread_mutex_unlock(&mxa);
