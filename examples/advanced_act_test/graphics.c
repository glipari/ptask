...
void draw_system_info (int s, int part, int prot, int p, int tipo_test, bool verbose) {
	...
	if( tipo_test <= PROT)
		...
		textout_ex(screen, font, "  deadline  ", 	XMIN+112*2, BASE1+10, FGC, 0);
		textout_ex(screen, font, "  cpu id  ", 		XMIN+112*3, BASE1+10, FGC, 0);
		textout_ex(screen, font, "  priority  ",	XMIN+112*4, BASE1+10, FGC, 0);
	}
	else textout_ex(screen, font, " --->  MODE  ACTIVE  = ", XMIN, BASE1+10, FGC, 0);
	...
	if( tipo_test == 5 ){ // checking for modeTest
		textout_ex(screen, font, "KEY [ Z ] to active DEFAULT mode", 5, YWIN-40, 10, 0);
		textout_ex(screen, font, "KEY [ X ] to active MODE_A", 5, YWIN-30, 10, 0);
		textout_ex(screen, font, "KEY [ C ] to active MODE_B", 5, YWIN-20, 10, 0);
	}
	else if ( tipo_test == 3 )  {
		textout_ex(screen, font, "KEY [ A ] to activate tasks whit act_flag = 0", 5, YWIN-20, 10, 0);
		textout_ex(screen, font, "KEY [ M ] to terminate tasks and show measure ", 5, YWIN-30, 10, 0);

	}
	else textout_ex(screen, font, "KEY [1-9] to activate tasks", 5, YWIN-20, 10, 0);
...
DIALOG box_modification[] =
	{
		...
		{ d_box_proc,		XMIN, 		TOP1-30, 	560, 	40, 	FGC, 	BGC, 	0, 0, 0, 0, NULL, NULL, NULL },

		{ d_text_proc, 		XMIN+10, 	TOP1-27, 	50, 	20, 	FGC, 	BGC,	0, 0, 0, 0, "index task", NULL, NULL },
		{ d_edit_proc,		XMIN+35, 	TOP1-12,	18, 	20, 	0, 	15, 	0, 0, 2, 0,	index_modification, NULL, NULL},

		{ d_text_proc, 		XMIN+170, 	TOP1-27, 	50, 	20, 	FGC, 	BGC,	0, 0, 0, 0, "select param", NULL, NULL },
		{ d_list_proc, 		XMIN+170, 	TOP1-15, 	100, 	20, 	FGC, 	0, 	0, 0, 0, 0, listboxModification_getter, NULL, NULL },

		{ d_text_proc, 		XMIN+340, 	TOP1-27, 	50, 	20, 	FGC, 	BGC,	0, 0, 0, 0, "insert value", NULL, NULL },
		{ d_edit_proc,		XMIN+360, 	TOP1-12,	50, 	20, 	0, 		15, 	0, 0, 9, 0,	value_modification, NULL, NULL},

		{ d_button_proc,	XMAX-50, 	TOP1-22, 	40, 	25, 	10, 0, 0, D_EXIT, 0, 0, "OK", NULL, NULL },
