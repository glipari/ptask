...
bool 	state_task[NUM_T_TEST];		/*  vector that means if a taski is active or not */

int	num_tasks;						/* number of tasks	*/
int scale = MILLI;					/* time scale		*/
int dipPeriodo = 0;					/* it means the 'dependency' of movement of ball from the period(0:no, 1:yes) */
...
/* given id_param_to_mod (index of a PARAMETER, call ptask_set_PARAMETER(idTask, value, ...) */
...
void draw_param_task(int i, int unit) {
	...
	textprintf_ex(screen, font, X_partenza, BASE1 + 2 * h_riga + h_riga * i, FGC, BGC, "%d", i);
	textprintf_ex(screen, font, X_partenza + spazio_colonna, BASE1 + 2 * h_riga + h_riga * i, FGC, BGC, "%d     ", ptask_get_period (i, unit));
	textprintf_ex(screen, font, X_partenza + spazio_colonna * 2, BASE1 + 2 * h_riga + h_riga * i, FGC, BGC, "%d     ", ptask_get_deadline (i, unit));
	textprintf_ex(screen, font, X_partenza + spazio_colonna * 3, BASE1 + 2 * h_riga + h_riga * i, FGC, BGC, "%d", ptask_get_processor (i));
	textprintf_ex(screen, font, X_partenza + spazio_colonna * 4, BASE1 + 2 * h_riga + h_riga * i, FGC, BGC, "%d", ptask_get_priority (i));
}
...
int	main(void)
{
    ...
    /* test a function for initializing a structure tpars with default parameters */
    ...
    if(num_tasks > NUM_T_TEST) {
    	fprintf(stderr, "Errore!!! non puoi superare i %d task!...modifica il file .dat o la define.\n", NUM_T_TEST);
    	exit(-1);
    }
    ...
    /******	test of functions ptask_param_PARAMETER(tpars tp, PARAMETER, int unit): set structure tp of task *****/
       ...
       for(i = 0; i < num_tasks; i++) {
			...
			if(measure_flag[i] == 1) ptask_param_measure(tp[i]);//sets to 1, so activates measure of exec time	*/
    ...
    for (i = 0; i < num_tasks; i++) {
    	...
    	else tid[i] = ptask_create_param(periodicBall_testParamDEP_PER, &tp[i]);

    	if(act_flag[i] == NOW) state_task[i]=ATTIVO;

    	printf("Creato (e a seconda del act:flag in tpars anche attivato) task%d\n",tid[i]);

    	if (!act_deferred && (act_flag[i] == DEFERRED)) act_deferred = true;
    	if (!act_measure  && (measure_flag[i] == 1))  act_measure = true;
    ...
    for (i = 0; i < num_tasks; i++) {
        		if ( state_task[i]!=ATTIVO || ( measure_flag[i] != 1) ) continue;
        		tspec wcet = ptask_get_wcet(i);
        		tspec acet = ptask_get_avg(i);
        		tspec total_exec = ptask_get_total(i); //total execution time performed by all the jobs since the task creation time.
        		printf("TASK %d: WCET = %ld\t ACET = %ld\t NINST=%d\t TOTEXEC=%ld\n",
        				tid[i],
    					tspec_to(&wcet, MICRO),
    					tspec_to(&acet, MICRO),
    					ptask_get_numinstances(i),
    					tspec_to(&total_exec, MICRO)
    				  );
    ...
    /*** PRINTING OF NEXT ACTIVATION TIME AND START TIME OF THE TASKS (first MAX_SAMPLE sample) ***/
    printf("*** Printing first %d activation_time/start_time(ms) of the tasks ***\n", MAX_SAMPLE);
    for (i = 0; i < num_tasks; i++) {

			if ( state_task[i]!= ATTIVO) continue;

			printf("TASK %d:\n", i);
			for (k = 1; k < MIN(MAX_SAMPLE, sample[i]); k++) {

					printf("\tnext_at[%d]-next_at[%d] = %ld\tstart_time[%d]-start_time[%d] = %ld\n", k, k-1,
							v_next_at[i][k]-v_next_at[i][k-1], k, k-1,
