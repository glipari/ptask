...
ptask taskbody(ptime work_time, int unit)
{
    ...
    for(;; job++) {
        if(dle_chkpoint() != 0) {
            ...
            printf("Task %d returned from a longjmp :\n\tCurrent time %ld | Deadline %ld\n", idx, ptask_gettime(MILLI), tspec_to_rel(&ptask_get_task(idx)->dl, MILLI));
            continue;
        }
        printf("Task %d starting for the %i time\n", idx, job);
        dle_timer_start(); //arming the deadline exception timer timer
        if((idx % 2) == 1 && job > 2) work_for(work_time * 3, unit);
        else work_for(work_time, unit);    
...
static int start_task(int unit, int period, int deadline, int priority, void (*task_body)(void))
{
    tpars param;
